...
import type { FeedPage } from "@/lib/achievements/application/feed";
...
export function FeedList({ initialPage, initialError = null }: FeedListProps) {
  ...
  const refreshFeed = useCallback(async (opts?: { silent?: boolean }) => {
    ...
    setError(null);
    const pageResult = await loadFollowingUnlockFeed({ limit: 20 });
    if (pageResult.isErr()) {
  ...
  async function loadMore() {
    ...
    setLoading(true);
    setError(null);
    const pageResult = await loadFollowingUnlockFeed({
      limit: 20,
      cursor,
    });
