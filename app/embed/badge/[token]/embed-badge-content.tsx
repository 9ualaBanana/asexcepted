...
export async function EmbedBadgeContent({ params }: Props) {
  ...
  const liveModelUrl =
    badge.isModelBadge && badge.iconAssetPath
      ? await createSignedBadgeModelUrl(badge.iconAssetPath)
      : null;
      ...
      <div className="h-[min(88vmin,20rem)] w-[min(88vmin,20rem)] max-h-[90dvh] max-w-[90dvw]">
        {liveModelUrl ? (
          <BadgeGltfViewer
            ...
            previewSrc={badge.renderSrc}
            className="p-1"
            float
            motionSeed={payload.achievementId}
            initialYaw={badge.iconModelYaw}
            initialPitch={badge.iconModelPitch}
          />
        ) : (
          <BadgeParallaxViewer
            src={badge.renderSrc}
